    ...
    try {
        ...
        if (cart) {
            for (const prod of arr){
                ...
                if(ok){
                    const indexProd = cart.products.findIndex((product) => product.id_prod._id.toString() === prod.productId)
                    console.log(indexProd)
                    indexProd === -1
                    ? cart.products.push({id_prod:prod.productId, quantity:prod.quantity})
                    : cart.products[indexProd].quantity = prod.quantity
                }else{
                    res.status(404).send(`Error en encontrar Producto ${prod}`)
                    break;
                }
            }
            await cartModel.findByIdAndUpdate(id, cart)
            res.status(200).send({ respuesta:"Productos actualizados", mensaje: cart})
        } else {
